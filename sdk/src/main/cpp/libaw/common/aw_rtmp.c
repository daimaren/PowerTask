...
int aw_rtmp_open(aw_rtmp_context *ctx){
    ...
    if (strlen(ctx->rtmp_url) <= 0 || strlen(ctx->rtmp_url) > 255) {
        AWLog("[error ] AVStreamPush rtmp setup url = %s\n", ctx->rtmp_url);
        return 0;
    }
...
int aw_rtmp_close(aw_rtmp_context *ctx){
    AWLog("AVStreamPush rtmp closing.......\n");
    if (aw_is_rtmp_opened(ctx)) {
        signal(SIGPIPE, SIG_IGN);
        RTMP_Close(ctx->rtmp);
        RTMP_Free(ctx->rtmp);
        ctx->rtmp = NULL;
        AWLog("AVStreamPush rtmp closed.......\n");
...
int aw_rtmp_write(aw_rtmp_context *ctx, const char *buf, int size){
    if (!aw_is_rtmp_opened(ctx)) {
        AWLog("[error] AVStreamPush rtmp writing but rtmp is not open");
        return 0;
    }
    signal(SIGPIPE, SIG_IGN);
