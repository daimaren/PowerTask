...
status_t CainMediaPlayer::prepareSave() {
    if (mediaPlayer == nullptr) {
        return NO_INIT;
    }
    if (mPrepareSaveSync) {
    ...
    mPrepareSaveSync = true;
    status_t ret = mediaPlayer->prepareSave();
    if (ret != NO_ERROR) {
        return ret;
    }
    if (mPrepareSaveSync) {
        mPrepareSaveSync = false;
    }
    return mPrepareSaveStatus;
}

status_t CainMediaPlayer::prepareSaveAsync() {
    if (mediaPlayer != nullptr) {
        return mediaPlayer->prepareSaveAsync();
    }
    return INVALID_OPERATION;
}

void CainMediaPlayer::startSave() {
    if (mediaPlayer != nullptr) {
        mediaPlayer->startSave();
    }
}
